...
// Pricing SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PricingOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PricingOmni::structfix($now, Struct::undef()))
                        !== json_encode(PricingOmni::structfix($val, Struct::undef()))) {
