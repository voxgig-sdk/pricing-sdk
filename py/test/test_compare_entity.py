    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PRICING_TEST_COMPARE_ENTID JSON to run live")
        client = setup["client"]
        ...
        compare_ref01_data_dt0_loaded = compare_ref01_ent.load(compare_ref01_match_dt0, None)
        assert compare_ref01_data_dt0_loaded is not None
...
def _compare_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PRICING_TEST_COMPARE_ENTID"))
