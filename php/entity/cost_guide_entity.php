...
class CostGuideEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CostGuideEntity($this->_client, $opts);
    }

    /**
     * @param CostGuide|array $args CostGuide data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PricingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CostGuide|array The current CostGuide data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CostGuide fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PricingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CostGuide fields).
     */
    ...
    /**
     * Load a single CostGuide.
     *
     * @param CostGuideLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CostGuideLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CostGuide|array The loaded CostGuide as an assoc-array at the
     *   SDK boundary; throws PricingError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PricingHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
