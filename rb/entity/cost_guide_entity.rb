...
class CostGuideEntity
  ...
  def make
    opts = @_entopts.dup
    CostGuideEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PricingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CostGuide, Hash] the current CostGuide data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PricingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CostGuide fields)
  def match_get
  ...
  #
  # @param reqmatch [CostGuideLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CostGuide.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CostGuide, Hash] the loaded CostGuide; raises PricingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PricingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
