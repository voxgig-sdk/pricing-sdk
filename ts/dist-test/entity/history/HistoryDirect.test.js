...
(0, node_test_1.describe)('HistoryDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-history', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-history', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['PRICING_TEST_HISTORY_ENTID'];
