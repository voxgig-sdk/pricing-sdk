...
{
    private static function utility(): PricingUtility
    {
        return new PricingUtility();
    }
    ...
    private static function ctx(array $over = []): PricingContext
    {
        ...
        $ctx->op = new PricingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PricingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PricingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PricingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PricingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PricingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PricingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PricingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PricingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PricingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = PricingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PricingSpec([]),
            'response' => null,
            'result' => new PricingResult([]),
        ]);
        [, $err] = PricingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PricingSpec(['step' => 's']),
            'response' => new PricingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PricingResult([]),
        ]);
        [, $err] = PricingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PricingSpec(['step' => 's']),
            'response' => new PricingResponse(self::resp(200, ['v' => 1])),
            'result' => new PricingResult([]),
        ]);
        [, $err] = PricingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PricingSpec(['step' => 's']),
            'response' => new PricingResponse(self::resp(200, ['v' => 2])),
            'result' => new PricingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PricingMakeResponse::call($ctx);
    ...
    {
        $preset = new PricingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PricingSpec([]),
            'response' => new PricingResponse([]),
            'result' => new PricingResult([]),
        ...
        [$got, $err] = PricingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PricingResult([])]);
        [, $err] = PricingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PricingSpec([]), 'result' => null]);
        [, $err] = PricingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PricingSpec(['step' => 's']),
            'result' => new PricingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PricingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PricingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PricingSpec(['step' => 's']),
            'result' => new PricingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PricingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PricingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PricingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PricingSpec([]),
            'result' => new PricingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PricingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PricingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PricingError('boom', 'boom');
        $utility->fetcher = function (PricingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PricingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PricingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PricingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PricingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PricingResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PricingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PricingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PricingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PricingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PricingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PricingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PricingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PricingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PricingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PricingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PricingResult(['ok' => false])]);
        $this->expectException(PricingError::class);
        PricingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PricingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PricingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PricingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PricingBaseFeature();
        $b = new PricingBaseFeature();
        PricingFeatureAdd::call($ctx, $a);
        PricingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PricingBaseFeature
    {
        $f = new PricingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PricingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PricingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PricingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PricingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PricingResponse(['headers' => null]),
            'result' => new PricingResult([]),
        ]);
        PricingResultHeaders::call($ctx);
