...
local PricingEntity = {}
PricingEntity.__index = PricingEntity


function PricingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PricingEntity)
  self._name = "pricing"
  self._client = client
...
function PricingEntity:get_name()
  return self._name
end


function PricingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PricingEntity.new(self._client, opts)
end
...
function PricingEntity:mark_deleted()
  self._deleted = true
end


function PricingEntity:deleted()
  return true == self._deleted
end


function PricingEntity:data_set(args)
  if args ~= nil then
...
function PricingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PricingEntity:match_set(args)
  if args ~= nil then
...
function PricingEntity:match_get()
...
function PricingEntity:stream(action, args, callopts)
...
---@param reqmatch PricingLoadMatch
---@param ctrl? table
---@return Pricing
---@return string? err
function PricingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PricingEntity:_run_op(ctx, post_done)
  local utility = self._utility
