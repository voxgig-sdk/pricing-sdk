...
local CostGuideEntity = {}
CostGuideEntity.__index = CostGuideEntity


function CostGuideEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CostGuideEntity)
  self._name = "cost_guide"
  self._client = client
...
function CostGuideEntity:get_name()
  return self._name
end


function CostGuideEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CostGuideEntity.new(self._client, opts)
end
...
function CostGuideEntity:mark_deleted()
  self._deleted = true
end


function CostGuideEntity:deleted()
  return true == self._deleted
end


function CostGuideEntity:data_set(args)
  if args ~= nil then
...
function CostGuideEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CostGuideEntity:match_set(args)
  if args ~= nil then
...
function CostGuideEntity:match_get()
...
function CostGuideEntity:stream(action, args, callopts)
...
---@param reqmatch CostGuideLoadMatch
---@param ctrl? table
---@return CostGuide
---@return string? err
function CostGuideEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CostGuideEntity:_run_op(ctx, post_done)
  local utility = self._utility
