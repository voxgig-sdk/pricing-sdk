// Command Pricing-cli is a CLI + REPL for the Pricing SDK,
...
const prompt = "pricing"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "compare cost_guide coverage discover history mcp pricing tco usage"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PRICING_APIKEY (api key), PRICING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PRICING_APIKEY carries the API key and
	// PRICING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PRICING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PRICING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPricingSDK(opts)
