...
class FtCtrl extends PricingControl
{
...
class FtClient
{
    ...
    private PricingUtility $utility;

    public function __construct(PricingUtility $utility, array $options)
    {
    ...
    public function get_utility(): PricingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PricingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PricingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PricingConfig::make_feature($name);
    ...
    private static function buildUrl(PricingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PricingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PricingResponse($fetched);
        }
    ...
    private function fail(PricingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PricingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
