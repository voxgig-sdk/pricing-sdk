...
(0, node_test_1.describe)('CompareDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-compare', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-compare', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['PRICING_TEST_COMPARE_ENTID'];
