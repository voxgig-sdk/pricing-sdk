...
The Lua SDK for the Pricing API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Compare()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/pricing-sdk/releases)),
...
```lua
local compare, err = client:Compare():load({ slug = "example_slug" })
...
```lua
local compare, err = client:Compare():load({ slug = "slug" })
```


### CostGuide

Create an instance: `local cost_guide = client:CostGuide(nil)`
...
```lua
local cost_guide, err = client:CostGuide():load({ id = "cost_guide_id" })
```


### Coverage

Create an instance: `local coverage = client:Coverage(nil)`
...
```lua
local coverage, err = client:Coverage():load({ slug = "slug" })
```


### Discover

Create an instance: `local discover = client:Discover(nil)`
...
```lua
local discover, err = client:Discover():load()
```
...
```lua
local history, err = client:History():load({ id = "history_id" })
```
...
Create an instance: `local pricing = client:Pricing(nil)`
...
| `name` | `string` |  |
| `positioning` | `table` | Starting price vs category median/min/max + sample size. |
| `priceRange` | `table` |  |
...
```lua
local pricing, err = client:Pricing():load({ id = "pricing_id" })
```


### Tco

Create an instance: `local tco = client:Tco(nil)`
...
```lua
local tco, err = client:Tco():create({
...
```
lua/
├── pricing_sdk.lua    -- Main SDK module
...
The main module (`pricing_sdk`) exports the SDK constructor
