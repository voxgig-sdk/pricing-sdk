...
const TEST_SEED = { "entity": { "compare": { "test01": { "id": "test01" } }, "cost_guide": { "test01": { "id": "test01" } }, "coverage": { "test01": { "id": "test01" } }, "discover": { "test01": { "id": "test01" } }, "history": { "test01": { "id": "test01" } }, "mcp": { "test01": { "id": "test01" } }, "pricing": { "test01": { "id": "test01" } }, "tco": { "test01": { "id": "test01" } }, "usage": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PricingSDK\b/.test(code) ||
        /\bPricingSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PricingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PricingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
