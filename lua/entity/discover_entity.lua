...
local DiscoverEntity = {}
DiscoverEntity.__index = DiscoverEntity


function DiscoverEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DiscoverEntity)
  self._name = "discover"
  self._client = client
...
function DiscoverEntity:get_name()
  return self._name
end


function DiscoverEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DiscoverEntity.new(self._client, opts)
end
...
function DiscoverEntity:mark_deleted()
  self._deleted = true
end


function DiscoverEntity:deleted()
  return true == self._deleted
end


function DiscoverEntity:data_set(args)
  if args ~= nil then
...
function DiscoverEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DiscoverEntity:match_set(args)
  if args ~= nil then
...
function DiscoverEntity:match_get()
...
function DiscoverEntity:stream(action, args, callopts)
...
---@param reqmatch DiscoverLoadMatch
---@param ctrl? table
---@return Discover
---@return string? err
function DiscoverEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DiscoverEntity:_run_op(ctx, post_done)
  local utility = self._utility
