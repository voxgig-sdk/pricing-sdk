...
// Pricing SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class PricingMakeError
{
    public static function call(?PricingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PricingContext([], null);
        }
        $op = $ctx->op ?? new PricingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PricingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PricingError) ? $err->msg : (string)$err;
        $msg = "PricingSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new PricingError('', $msg, $ctx);
