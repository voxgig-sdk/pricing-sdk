...
The TypeScript SDK for the Pricing API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Compare()` — each with a small set of operations (`load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `PricingSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `PricingSDK.test(testopts?, sdkopts?)` | `PricingSDK` | Create a test-mode client. |
...
| `client` | `client(): PricingSDK` | Return the parent SDK client. |
...
Create an instance: `const cost_guide = client.CostGuide()`
...
```ts
const cost_guide = await client.CostGuide().load({ id: 'cost_guide_id' })
```
...
Create an instance: `const discover = client.Discover()`
...
```ts
const discover = await client.Discover().load()
```
...
```ts
const history = await client.History().load({ id: 'history_id' })
```
...
Create an instance: `const pricing = client.Pricing()`
...
```ts
const pricing = await client.Pricing().load({ id: 'pricing_id' })
```


### Tco

Create an instance: `const tco = client.Tco()`
...
```ts
const tco = await client.Tco().create({
  slug: 'example_slug',
})
```
