...
[MCP](https://modelcontextprotocol.io) server exposing the Pricing SDK as
two agent tools — `pricing_list` and `pricing_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pricing-mcp)
make build

# 2. Provide credentials via the environment
export PRICING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pricing \
  -- /absolute/path/to/pricing-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pricing-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pricing_list: first page of records
{ "entity": "compare" }
{ "entity": "compare", "query": { } }

// pricing_load: one record by id
{ "entity": "compare", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pricing-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pricing \
     -- "$PWD"/dist/*/pricing-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pricing_list` and `pricing_load` tools now appear
   in new sessions. Ask the agent to *"list compare using pricing"*
   and it calls `pricing_list` with `{"entity":"compare"}`.
...
```sh
export PRICING_APIKEY=sk_live_xxx            # API key
export PRICING_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pricing_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pricing_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PRICING_APIKEY` | API key sent with every request. |
| `PRICING_BASE` | Optional override of the API base URL. |

### Entities

The 9 entities valid as the `entity` argument:

compare | cost_guide | coverage | discover | history | mcp | pricing | tco | usage

### Smoke test via HTTP (raw JSON-RPC)

```sh
./pricing-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pricing_load","arguments":{"entity":"compare","query":{"id":1}}}}'
```
