...
type Args struct {
	Entity string         `json:"entity" jsonschema:"compare | cost_guide | coverage | discover | history | mcp | pricing | tco | usage"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PricingSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "pricing_list",
		Description: "List records from Pricing. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "pricing_load",
		Description: "Load a single record from Pricing. " +
...
func runOp(client *sdk.PricingSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PricingSDK, name string) (sdk.PricingEntity, error) {
	switch strings.ToLower(name) {
	case "compare":
		return client.Compare(nil), nil
	case "cost_guide":
		return client.CostGuide(nil), nil
	case "coverage":
		return client.Coverage(nil), nil
	case "discover":
		return client.Discover(nil), nil
	case "history":
		return client.History(nil), nil
	case "mcp":
		return client.Mcp(nil), nil
	case "pricing":
		return client.Pricing(nil), nil
	case "tco":
		return client.Tco(nil), nil
	case "usage":
