...
import (
	...
	omni "github.com/voxgig-sdk/pricing-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PricingError); is {
				out := map[string]any{
					"name":    "PricingError",
...
func omniCtx(arg any, client *sdk.PricingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PricingSDK); is {
			client = live
