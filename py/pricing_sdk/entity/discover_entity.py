...
class DiscoverEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DiscoverEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Discover:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Discover().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
