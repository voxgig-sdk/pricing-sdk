...
-- by passing it.
function PricingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PricingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PricingSDK: graphql: " .. msg
    res.graphql = errors
  end
