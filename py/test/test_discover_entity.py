...
class TestDiscoverEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Discover(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _discover_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "discover." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PRICING_TEST_DISCOVER_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        discover_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.discover")))
        discover_ref01_data = None
        if len(discover_ref01_data_raw) > 0:
            discover_ref01_data = helpers.to_map(discover_ref01_data_raw[0][1])

        # LOAD
        discover_ref01_ent = client.Discover(None)
        discover_ref01_match_dt0 = {}
        discover_ref01_data_dt0_loaded = discover_ref01_ent.load(discover_ref01_match_dt0, None)
        assert discover_ref01_data_dt0_loaded is not None



def _discover_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/discover/DiscoverTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PRICING_TEST_DISCOVER_ENTID"))
