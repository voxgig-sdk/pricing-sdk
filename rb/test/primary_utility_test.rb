...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = PricingOmni.make_runner(TEST_JSON_FILE, PricingSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : PricingOmni.errify(err)
    end
