...
boru-driven command-line client **and** interactive REPL for the Pricing
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pricing-cli)
make build
...
./pricing-cli load 1 compare            # {id:1} shorthand
./pricing-cli load '{id:1}' compare       # explicit match map

# 5. Override the API base URL for a single call
PRICING_BASE=https://api.example.com ./pricing-cli load 1 compare

# 6. No arguments -> interactive REPL
./pricing-cli
pricing> load 1 compare
pricing> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pricing-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pricing-cli load 1 compare
   ```
...
```sh
./pricing-cli load 1 compare          # scalar shorthand for {id:1}
./pricing-cli load '{id:1}' compare     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pricing>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `compare`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pricing-cli`. |
...
(`./pricing-cli <expr>`) and interactively (the REPL), and expressions compose the
