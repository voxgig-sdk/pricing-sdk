...
module PricingUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = PricingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PricingResponse.new(fetched)
    else
