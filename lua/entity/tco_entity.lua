...
local TcoEntity = {}
TcoEntity.__index = TcoEntity


function TcoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TcoEntity)
  self._name = "tco"
  self._client = client
...
function TcoEntity:get_name()
  return self._name
end


function TcoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TcoEntity.new(self._client, opts)
end
...
function TcoEntity:mark_deleted()
  self._deleted = true
end


function TcoEntity:deleted()
  return true == self._deleted
end


function TcoEntity:data_set(args)
  if args ~= nil then
...
function TcoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TcoEntity:match_set(args)
  if args ~= nil then
...
function TcoEntity:stream(action, args, callopts)
...
---@param reqdata TcoCreateData
---@param ctrl? table
---@return Tco
---@return string? err
function TcoEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function TcoEntity:_run_op(ctx, post_done)
  local utility = self._utility
