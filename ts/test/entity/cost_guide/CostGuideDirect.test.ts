...
describe('CostGuideDirect', async () => {
  ...
  test('direct-load-cost_guide', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-cost_guide', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PRICING_TEST_COST_GUIDE_ENTID']
